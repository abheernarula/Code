...
layout = [
    [sg.Text('Input Folder'), sg.Input(key='input_folder'), sg.FolderBrowse()],
    [sg.Text('Rules Folder'), sg.Input(key='rules_dir'), sg.FolderBrowse()],
    [sg.Text('Output Folder'), sg.Input(key='output_folder'), sg.FolderBrowse()],
    [sg.Checkbox('Preprocess Input', key='preprocess')],
    [sg.Checkbox('Vendor', key='isVendor'), 
     sg.Checkbox('Customer', key='isCustomer'), 
     sg.Checkbox('Material', key='isMaterial')],
    [sg.Text('Material Type (optional):'), sg.Input(key='materialType')],
    [sg.Button('Run'), sg.Button('Exit')],
    [sg.Output(size=(80, 20))]
]


window = sg.Window('Data Processing GUI', layout)
...
while True:
    ...
    if event == 'Run':
        try:
            # Material type is mandatory only if 'Material' checkbox is checked
            if values['isMaterial'] and not values['materialType'].strip():
                print("[ERROR] 'Material Type' is required when 'Material' is selected.")
            else:
                result = run_pipeline(
                    values['input_folder'],     # Input folder
                    values['isVendor'],         # Vendor checkbox
                    values['isCustomer'],       # Customer checkbox
                    values['isMaterial'],       # Material checkbox
                    values['materialType'],     # Material type (optional)
                    values['rules_dir'],        # Rules folder
                    values['output_folder'],    # Output folder
                    values['preprocess']        # Preprocess checkbox
                )
                print(result)
        except Exception as e:
            import traceback
