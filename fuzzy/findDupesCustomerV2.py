...
def find_duplicates(df,
    ...
    mask_non_null = pd.notna(gst_vals) & pd.notna(vat_vals)
    for _, group in df_result[mask_non_null].groupby([gst_col, vat_col]):
        print(f'done - {_}')
    ...
    mask_null = pd.isna(gst_vals) & pd.isna(vat_vals)
